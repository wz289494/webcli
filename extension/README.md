# WebCLI Browser Bridge Extension

The extension connects Chrome tabs to the local WebCLI bridge server. It uses Chrome
extension APIs only as a transport and browser-control layer for explicit CLI
commands.

## Permission Notes

- `debugger`: sends CDP commands to WebCLI-controlled or bound tabs.
- `tabs` / `tabGroups`: manages the dedicated WebCLI automation container and
  reports selected tab metadata back to the CLI.
- `cookies`: reads cookies for browser-backed adapters that need authenticated
  fetches.
- `downloads`: surfaces download lifecycle to `webcli browser wait download`.
  The extension observes started / in-progress / completed / failed downloads so
  the CLI can wait for a file triggered by an automation command. WebCLI
...
> (started / in-progress / completed / failed) to the WebCLI command-line tool,
