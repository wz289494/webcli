// @ts-nocheck
/**
 * WebCLI bridge extension background worker.
 *
 * This source mirrors the currently working runtime logic and connects directly
 * to the local Python bridge server exposed by `webcli bridge serve`.
 */

const PY_BRIDGE_PORT = 8765;
const PY_BRIDGE_HOST = "127.0.0.1";
const PY_BRIDGE_WS_URL = `ws://${PY_BRIDGE_HOST}:${PY_BRIDGE_PORT}`;
...
async function ensureAttached(tabId, aggressiveRetry = false) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTACH_RETRIES; attempt++) {
    try {
      ...
      if (attempt < MAX_ATTACH_RETRIES) {
        console.warn(`[webcli] attach attempt ${attempt}/${MAX_ATTACH_RETRIES} failed: ${lastError}, retrying in ${RETRY_DELAY_MS}ms...`);
  ...
  if (lastError) {
    ...
    console.warn(`[webcli] attach failed for tab ${tabId}: url=${finalUrl}, windowId=${finalWindowId}, error=${lastError}`);
...
const CONTEXT_ID_KEY = "webcli_context_id_v1";
...
async function connectAttempt() {
  ...
  try {
    ...
    if (isDaemonSocketActive()) return;
    thisWs = new WebSocket(PY_BRIDGE_WS_URL);
    ws = thisWs;
  } catch {
    scheduleReconnect();
    return;
  ...
  thisWs.onopen = () => {
    ...
    _origLog("[webcli] Connected to WebCLI bridge server");
  };
  thisWs.onmessage = async (event) => {
    ...
    try {
      ...
      console.error("[webcli] Message handling error:", err);
    }
  };
  thisWs.onclose = () => {
    if (ws !== thisWs) return;
    console.log("[webcli] Disconnected from WebCLI bridge server");
    ws = null;
    scheduleReconnect();
  };
  thisWs.onerror = () => {
...
const REGISTRY_KEY = "webcli_target_lease_registry_v2";
const LEASE_IDLE_ALARM_PREFIX = "webcli:lease-idle:";
const CONTAINER_TAB_GROUP_TITLE = {
  interactive: "WebCLI Browser",
  automation: "WebCLI Adapter"
};
...
function getSessionName(session) {
  ...
  if (!raw) throw new CommandFailure(
    ...
    "Pass a browser session name, e.g. webcli browser <session> <command>."
  );
  return raw;
}
...
async function ensureOwnedContainerTabGroupUnlocked(role, windowId, ids) {
  try {
    ...
    console.warn(`[webcli] Failed to mark ${role} tab group: ${err instanceof Error ? err.message : String(err)}`);
  }
}
...
async function ensureOwnedContainerWindowUnlocked(role, initialUrl, mode = "background") {
  ...
  console.log(`[webcli] Created owned ${role} window ${container.windowId} (start=${startUrl})`);
  ...
  if (existing) {
    if (!existing.owned) {
      throw new CommandFailure(
        ...
        `Session "${existing.session}" is bound to a user tab and does not own an WebCLI tab lease.`,
...
chrome.windows.onRemoved.addListener(async (windowId) => {
  ...
  for (const [leaseKey, session] of automationSessions.entries()) {
    if (session.windowId === windowId) {
      console.log(`[webcli] ${session.surface} container closed (session=${session.session})`);
      if (session.idleTimer) clearTimeout(session.idleTimer);
      automationSessions.delete(leaseKey);
      sessionTimeoutOverrides.delete(leaseKey);
      sessionWindowModeOverrides.delete(leaseKey);
...
chrome.tabs.onRemoved.addListener(async (tabId) => {
  ...
  for (const [leaseKey, session] of automationSessions.entries()) {
    if (session.preferredTabId === tabId) {
      ...
      sessionLifecycleOverrides.delete(leaseKey);
      scheduleIdleAlarm(leaseKey, IDLE_TIMEOUT_NONE);
      console.log(`[webcli] Session ${session.session} detached from tab ${tabId} (tab closed)`);
    }
  }
  await persistRuntimeState();
});
...
function initialize() {
  ...
  console.log("[webcli] WebCLI extension initialized");
}
...
async function handleCommand(cmd) {
  ...
  const leaseKey = getLeaseKey(session, "browser");
...
async function resolveTab(tabId, leaseKey, initialUrl) {
    ...
    try {
      ...
      if (session && !session.owned) {
        throw new CommandFailure(
          ...
          'Run "webcli browser bind" again on a debuggable http(s) tab.'
        );
      }
      if (session && !matchesSession && session.preferredTabId === null && isDebuggableUrl(tab.url)) {
        console.warn(`[webcli] Tab ${tabId} drifted to window ${tab.windowId}, moving back to ${session.windowId}`);
        try {
          ...
          console.warn(`[webcli] Failed to move tab back: ${moveErr}`);
        }
      } else if (!isDebuggableUrl(tab.url)) {
        console.warn(`[webcli] Tab ${tabId} URL is not debuggable (${tab.url}), re-resolving`);
      }
    } catch (err) {
      ...
      if (existingSession && !existingSession.owned) {
        ...
        throw new CommandFailure(
          ...
          'Run "webcli browser bind" again, then retry the command.'
        );
      }
      console.warn(`[webcli] Tab ${tabId} no longer exists, re-resolving`);
    }
  }
    ...
    try {
      ...
      if (!session.owned) {
        throw new CommandFailure(
          ...
          'Switch the tab to an http(s) page or run "webcli browser bind" on another tab.'
        );
      }
    } catch (err) {
      if (err instanceof CommandFailure) throw err;
  ...
  if (reuseTab?.id) {
    ...
    try {
      ...
      console.warn(`[webcli] data: URI was intercepted (${updated.url}), creating fresh tab`);
    } catch {
    }
  }
...
async function handleNavigate(cmd, leaseKey) {
  ...
  await new Promise((resolve) => {
    ...
    timeoutTimer = setTimeout(() => {
      timedOut = true;
      console.warn(`[webcli] Navigate to ${targetUrl} timed out after 15s`);
      finish();
    }, 15e3);
  });
  ...
  if (postNavigationSession && tab.windowId !== postNavigationSession.windowId) {
    console.warn(`[webcli] Tab ${tabId} drifted to window ${tab.windowId} during navigation, moving back to ${postNavigationSession.windowId}`);
    try {
      await chrome.tabs.move(tabId, { windowId: postNavigationSession.windowId, index: -1 });
      tab = await chrome.tabs.get(tabId);
    } catch (moveErr) {
      console.warn(`[webcli] Failed to recover drifted tab: ${moveErr}`);
    }
  }
...
async function handleTabs(cmd, leaseKey) {
  ...
  if (session && !session.owned && cmd.op !== "list") {
    return {
      ...
      error: `Session "${session.session}" is bound to a user tab; tab new/select/close requires an owned WebCLI session.`,
      errorHint: "Unbind the session first, or use a different session for owned WebCLI tabs."
    };
  }
  switch (cmd.op) {
    case "list": {
  ...
  if (session.owned) {
    ...
    if (tabId !== null) {
      ...
      if (hasOtherOwnedLease) {
        ...
        console.log(`[webcli] Released owned tab lease ${tabId} (session=${session.session}, surface=${session.surface}, ${reason})`);
      } else {
        try {
          ...
          console.log(`[webcli] Released owned tab lease ${tabId} as reusable placeholder (session=${session.session}, surface=${session.surface}, ${reason})`);
        } catch {
          await chrome.tabs.remove(tabId).catch(() => {
          });
          console.log(`[webcli] Released owned tab lease ${tabId} (session=${session.session}, surface=${session.surface}, ${reason})`);
      ...
      console.log(`[webcli] Released legacy owned window lease ${session.windowId} without closing container (session=${session.session}, surface=${session.surface}, ${reason})`);
    }
  } else if (session.preferredTabId !== null) {
    await safeDetach(session.preferredTabId);
    console.log(`[webcli] Detached borrowed tab lease ${session.preferredTabId} (session=${session.session}, surface=${session.surface}, ${reason})`);
  }
  automationSessions.delete(leaseKey);
  sessionTimeoutOverrides.delete(leaseKey);
  sessionWindowModeOverrides.delete(leaseKey);
...
async function handleBind(cmd, leaseKey) {
  ...
  resetWindowIdleTimer(leaseKey);
  console.log(`[webcli] Session ${getSessionFromKey(leaseKey)} explicitly bound to tab ${boundTab.id} (${boundTab.url})`);
